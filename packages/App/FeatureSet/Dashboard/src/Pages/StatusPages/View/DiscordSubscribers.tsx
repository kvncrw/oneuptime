...
const StatusPageDiscordSubscribers: FunctionComponent<
  PageComponentProps
> = (props: PageComponentProps): ReactElement => {
  ...
  const [
    isDiscordSubscribersEnabled,
    setIsDiscordSubscribersEnabled,
  ] = React.useState<boolean>(false);
  ...
  const fetchStatusPage: PromiseVoidFunction = async (): Promise<void> => {
    try {
      ...
      if (statusPage && statusPage.enableDiscordSubscribers) {
        setIsDiscordSubscribersEnabled(
  ...
  const teamsCsvColumns: Array<CSVColumn> = [
    {
      ...
      required: true,
      description: "Label for the Discord channel, used to manage this subscription",
    },
    {
      key: "webhookUrl",
  ...
  const handleBulkAddSubmit: () => Promise<void> = async (): Promise<void> => {
    ...
    for (let i: number = 0; i < csvRows.length; i++) {
      ...
      const channelName: string = row["channelName"] || "";

      try {
        const subscriber: StatusPageSubscriber = new StatusPageSubscriber();
        subscriber.discordIncomingWebhookUrl = URL.fromString(urlStr);
        subscriber.discordChannelName = channelName;
    ...
    const formFields: Array<ModelField<StatusPageSubscriber>> = [
      {
        ...
        title: "Discord Channel Name",
        description:
          "Label for the Discord channel, used to manage this subscription.",
      ...
      },
      {
        ...
        description: "Status page updates will be sent to this Discord channel.",
      ...
      },
      {
        ...
        stepId: "subscriber-info",
        description:
          "Send a notification to the Discord channel confirming the subscription.",
...

      {
        ...
        title: "Unsubscribe",
        stepId: "subscriber-info",
        description: "Unsubscribe this Discord channel from the status page.",
        fieldType: FormFieldSchemaType.Toggle,
        required: false,
        ...
        <>
          {!isDiscordSubscribersEnabled && (
            <Alert
              ...
              title="Discord subscribers are not enabled for this status page. Please enable it in Subscriber Settings"
            />
          )}
          <SubscriberNotificationWarnings statusPageId={modelId} />
          <ModelTable<StatusPageSubscriber>
            modelType={StatusPageSubscriber}
            id="table-discord-subscriber"
            name="Status Page > Discord Subscribers"
            userPreferencesKey="status-page-discord-subscribers-table"
            saveFilterProps={{
              tableId: "status-page-discord-subscribers-table",
            }}
            isDeleteable={true}
            ...
            query={{
              ...
              discordChannelName: new NotNull(),
            }}
            onBeforeCreate={(
            ...
            cardProps={{
              ...
              description:
                "Here are the list of Discord channels that have subscribed to the status page.",
              buttons: [
                {
                  title: "Add in Bulk",
            ...
            noItemsMessage={"No Discord subscribers found."}
            formSteps={[
              {
          ...
          {showBulkAddModal && (
            <Modal
              title="Add Discord Subscribers in Bulk"
              description="Upload a CSV file with Discord channel names and webhook URLs. Download the template to get started."
            ...
            >
              <div className="space-y-4">
                <CSVFileUpload
                  ...
                  templateFileName="discord-subscribers-template.csv"
                />
                <div className="flex items-center space-x-3 pt-2">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-500">
                      Send a notification to the Discord channels confirming the
                      subscription.
                    </div>
                  </div>
                </div>
...
export default StatusPageDiscordSubscribers;
